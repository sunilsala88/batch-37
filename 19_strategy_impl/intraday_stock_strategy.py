...
TOKEN = '8658989850:AAHaHFI5MCnQArWMgiqnZIJ4xT_oIklRAeE'
ids = '5563890177'
...
def strategy_condition(df,ticker,quantity):
    ...
    sell_condition=ema_now<sma_now and ema_prev>sma_prev
    ...
    if buy_condition:
        ...
        logger.info(f'SELL entry condition satisfied | {ticker} | qty={quantity}')
        ...
        try:
            ...
            if position_status==0:
                ...
                if exit_condition:
                    ...
                    ib.sleep(2)  # slight delay to allow order processing
                    strategy_condition(df,ticker,quantity)  # check if new entry condition is satisfied after exit
                else:
                ...
                if exit_condition:
                    ...
                    close_ticker_position(ticker)
                    ib.sleep(2)  # slight delay to allow order processing
                    strategy_condition(df,ticker,quantity)  # check if new entry condition is satisfied after
                else:
                    logger.info(f'No exit condition for short {ticker}')

        except Exception as e:
            logger.exception(f'Unexpected error processing {ticker}, skipping {e}')
            continue
