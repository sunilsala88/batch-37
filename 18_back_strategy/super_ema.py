...
class super_ema(Strategy):
    ...
    def next(self):
        ...
        if trend!=self.current_trend:
            ...
            # Exit on trend flip alone - don't wait for the lagging
            # daily EMA to agree before closing the existing position.
            if trend==1 and self.position.is_short:
                self.position.close()
            elif trend==-1 and self.position.is_long:
                self.position.close()
